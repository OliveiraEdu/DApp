...
def generate_asset_id():
    ...
    return f"{asset_name}#{DOMAIN}"

# Example usage:
ASSET_ID = generate_random_asset_name()
...
def create_and_setup_account(contract_address: str, user_account_short_id: str, filename: str, cid: str, public_key: str):
    try:
        ...
        params = params + integration_helpers.argument_encoding(ADMIN_ACCOUNT_ID)  # admin account id
        params = params + integration_helpers.argument_encoding(user_account_short_id)  # user account name
        params = params + integration_helpers.argument_encoding(f"{user_account_short_id}@{DOMAIN}")  # user account id
        ...
        params = params + integration_helpers.argument_encoding("set up balance")  # description
        params = params + integration_helpers.argument_encoding("100")  # amount
        params = params + integration_helpers.argument_encoding(filename)  # key for user detail (filename)
        params = params + integration_helpers.argument_encoding(cid)  # value for user detail (CID)
        params = params + integration_helpers.argument_encoding(public_key)  # public key of user
...
def main():
    try:
        ...
        sets_asset(address, asset_id, str(amount))
        # print(response)
        # if not response:
        #     print("Failed to set asset.")
        #     return
        ...
        if file_cid:
            ...
            success = create_and_setup_account(address, user_account_short_id, os.path.basename(local_file_path), file_cid, user_private_key)
            if success:
