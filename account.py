...
def get_first_four_bytes_of_keccak(data):
    k = keccak.new(digest_bits=256)
    k.update(data)
    return k.hexdigest()[:8]

def left_padded_address_of_param(param_index, total_params):
    return "0" * (64 - len(hex(param_index)[2:])) + hex(param_index)[2:]

def argument_encoding(arg):
    return binascii.hexlify(arg.encode('utf-8')).decode('utf-8').ljust(64, '0')

def get_engine_receipts_address(hash):
    # Placeholder implementation
    return "address_from_receipt"

def get_engine_receipts_result(hash):
    # Placeholder implementation
...
def create_contract():
    ...
    for status in net.tx_status_stream(tx):
        print(f"create_contract status: {status}")
    return hex_hash
...
def create_account(address):
    ...
    net.send_tx(tx)
    for status in net.tx_status_stream(tx):
        print(f"create_account status: {status}")
    hex_hash = binascii.hexlify(IrohaCrypto.hash(tx))
    return hex_hash
...
def get_account(address):
    ...
    for x in range(no_of_param):
        params = params + left_padded_address_of_param(x, no_of_param)
    params = params + argument_encoding("admin2@test")  # account id
    tx = iroha.transaction(
        [
            iroha.command(
                "CallEngine", caller=ADMIN_ACCOUNT_ID, callee=address, input=params
    ...
    net.send_tx(tx)
    for status in net.tx_status_stream(tx):
        print(f"get_account status: {status}")
    hex_hash = binascii.hexlify(IrohaCrypto.hash(tx))
    return hex_hash
